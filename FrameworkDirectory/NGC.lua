...
local Packets: Packets = {
	Promise = require(Module.Parent:WaitForChild("Packets").Promise);
} 
...
function Framework:ConnectBridge( ): RBXScriptSignal?
	local function initializationBridge(module: ModuleScript)
		...
		Promise.new(function(resolve, onCancel, reject)
	...
	return {
		ConnectServices = function(Folder: Folder | Instance) 
	        ...
	        for _Index, Elements in ipairs(Folder:GetChildren()) do table.insert(ListOfServices, Elements); initializationBridge(Elements) end
...
function Framework:Create(Data: table)
	...
	if (RunService:IsClient()) then
		Promise.try(function(resolve, onCancel, reject)
